...
function genererBoutons(questionId) {
  const options = [
  ...
  return options.map(o =>
    `<button type="button" class="star-btn" onclick="selectNote('${questionId}', ${o.val})">
      <span class="emoji">${o.emoji}</span>${o.label}
    </button>`
  ).join('');
}

function genererQuestionsHTML(questions, numDepart) {
  return questions.map((q, i) => `
    <div class="question-card">
      <div class="question-label">
        <span class="question-num">${numDepart + i}</span>${q.label}
      </div>
      <div class="stars" id="stars-${q.id}-${i}">
...
router.get('/:clientId', (req, res) => {
  ...
  db.get(`SELECT c.*, o.id as operation_id, o.type_operation, a.nom as agence_nom
    ...
    const questionsHTML = genererQuestionsHTML(questionnaire.questions, 1);
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .intro-card p { color: #555; font-size: 14px; line-height: 1.5; }
    .badge-type { display: inline-block; background: #4d553d; color: white; border-radius: 20px; padding: 3px 12px; font-size: 12px; margin-top: 10px; }
    ...
    textarea { width: 100%; padding: 12px; border: 2px solid #ddd; border-radius: 8px; font-size: 14px; font-family: Arial, sans-serif; resize: vertical; min-height: 90px; transition: border 0.2s; }
    textarea:focus { outline: none; border-color: #4d553d; }
    .submit-btn { width: 100%; padding: 16px; background: #4d553d; color: white; border: none; border-radius: 10px; font-size: 17px; font-weight: bold; cursor: pointer; margin-top: 8px; transition: background 0.2s; }
    .submit-btn:hover { background: #3a4130; }
    .reclamation-link { display: block; text-align: center; margin-top: 20px; color: #a6aa9e; font-size: 13px; text-decoration: none; }
    .reclamation-link:hover { color: #4d553d; }
    .success-screen { display: none; text-align: center; background: white; border-radius: 10px; padding: 40px 20px; box-shadow: 0 2px 8px rgba(0,0,0,0.08); }
    .success-screen .check { font-size: 60px; margin-bottom: 16px; }
    .success-screen h2 { color: #4d553d; font-size: 22px; margin-bottom: 12px; }
...
<div class="container">
  <div class="intro-card">
    ...
    <p>Votre avis compte beaucoup pour la BCEG. Cette enquete prend moins de <strong>2 minutes</strong> et nous aide a ameliorer nos services.</p>
    <span class="badge-type">${questionnaire.titre}</span>
  </div>
...
<script>
  var notes = {};
  function selectNote(critereAvecIndex, val) {
    var parts = critereAvecIndex.split('-');
    var critere = parts.slice(0, parts.length - 1).join('_');
    notes[critere] = val;
    document.getElementById(critere).value = val;
    var container = document.getElementById('stars-' + critereAvecIndex.replace(/_/g, '-'));
    if (container) {
